---
description: New Script - Scaffold a new custom C++ script and register it in the script loader
---
...
1. Parse the script name (snake_case) and optional type from the user input.
   - Types: `command`, `player`, `world`, `creature`, `spell`. Default is `player`.

2. Use `write_to_file` to create `src/server/scripts/Custom/<name>.cpp` with appropriate boilerplate based on type:
...
3. Read `custom_script_loader.cpp` using `view_file` or `grep_search`.
4. Edit the file using `multi_replace_file_content` to add:
...
5. Report what was created and remind the user to build with `ninja -j4 scripts` (or via Visual Studio)
