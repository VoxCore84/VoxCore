---
description: Parse Packet Dump - Parse WowPacketParser output or raw packet hex and map to codebase handlers.
---
...
1. **Identify the opcode**: Extract the opcode name and/or number from the user input.
2. **Find the handler**: Search the codebase for the opcode using `grep_search`:
...
3. **Map packet fields**: Read the packet structure definition using `view_file` and list each field with:
