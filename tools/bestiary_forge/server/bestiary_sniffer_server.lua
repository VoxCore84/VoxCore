--[[
    BestiaryForge Server-Side Eluna Script

    Handles addon message requests from the BestiaryForge client addon:
...
local function HandleAggregateSubmit(player, payload)
    ...
    for chunk in spellData:gmatch("[^,]+") do
        ...
        if spellId and count and count > 0 then
            -- Insert or update in roleplay.bestiary_aggregated
            CharDBExecute(string.format(
                "INSERT INTO roleplay.bestiary_aggregated (creature_entry, spell_id, cast_count, last_reporter, last_seen) " ..
                "VALUES (%d, %d, %d, '%s', %d) " ..
...
-- Listen for addon messages from BestiaryForge clients
...
print("[BestiaryForge] Server-side Eluna sniffer loaded (v2: zone completeness + aggregation).")
