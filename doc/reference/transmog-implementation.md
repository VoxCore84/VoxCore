...
- **Empty slot detection**: IMAID == 0 (NOT byte[1]). byte[1] is always 0 in all observed packets
- **DisplayType IS the routing key** (confirmed by looking up every IMAID in Wago DB2):
...
- **Multi-spec**: UPDATE_SLOTS sends N*14 entries (one block per spec). Only first 14 used.
...
### Stale Data Detection (Mar 4 2026)
Layer 1 (`GetViewedOutfitSlotInfo`) returns currently-worn appearance for ALL slots, even slots the outfit doesn't define. HEAD and SHOULDER always carry stale data (never nil).
- **Pre-snapshot**: Before Layer 1, capture `GetSlotVisualInfo.appliedSourceID` for all slots (the pre-outfit equipped appearance)
- **Stale check**: After merge + hidden detection, if `merged[slot].transmogID == preSnapshot[slot]` and slot is NOT from Layer 2, the data is stale → replace with clear `{0, 0}`
- **Layer 2 exempt**: `layer2Slots` set tracks slots from SetPendingTransmog hooks (explicit user choices, never stale)
- **Edge case**: If outfit legitimately includes same appearance as currently worn, first apply clears it (false positive); second apply works because snapshot shows cleared slot
...
- `ChatHandler.cpp` — TMOG_BRIDGE prefix intercept, parses `slot.transmogID.option[.illusionID]` format, accepts `transmogID >= 0` (0 = clear)
- `TransmogrificationHandler.cpp` — deferred storage, MapClientSlotToEquipSlot(), FinalizeTransmogBridgePendingOutfit() with `bridgeOverriddenMask` + `bridgeOverrodeSecondary` + `bridgeClearedMask`, illusion merge via Enchants[0/1] with HasIllusion flag, post-validate `bridgeClearedMask` re-clears IgnoreMask, SendUpdateToPlayer+ClearUpdateMask flush in all 4 handlers, validateIllusion fix (TransmogIllusion DB2 check)
...
- Distribution zip: `C:/Dev/RoleplayCore/TransmogBridge.zip`
...
### Open Bugs Under Investigation (Mar 4 2026, session 36)
5 bugs found during testing, diagnostic build deployed:
...
### Diagnostic Build (deployed Mar 4 2026, session 36, NOT YET COMMITTED)
Files modified with diagnostic logging:
1. `TransmogrificationHandler.cpp` — post-reapply per-slot dump, HandleTransmogrifyItems entry+sync logs
2. `Player.cpp` — fillOutfitData canary (CLEAR-SKIP), _SyncTransmogOutfitsToActivePlayerData entry log
